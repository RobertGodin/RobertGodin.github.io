-- Solutions aux exercices du chapitre 6 sous forme de script SQL*plus
-- N.B. Certaines solutions SQL2 ne sont pas compatibles avec le 
-- dialecte Oracle !
...
SELECT	DISTINCT Commande.noCommande, LD.noLivraison
FROM 		(Commande NATURAL LEFT OUTER JOIN 
 		 (LigneCommande NATURAL JOIN DétailLivraison)) AS LD
/
-- Solution avec le dialecte Oracle
SELECT 	Commande.noCommande, LD.noLivraison
FROM		Commande,
 		(SELECT	DISTINCT L.noCommande, D.noLivraison
 		 FROM 	LigneCommande L, DétailLivraison D
 		 WHERE	L.noCommande = D.noCommande AND
				L.noArticle = D.noArticle) LD
WHERE		Commande.noCommande = LD.noCommande (+)
/
...
-- (voir volume 2, chaitre 8, exercice 2)
